...
- test.js status=M lines=150 max=82
...
the gate catching a fabricated report against real upstream history.

The one thing this report gets wrong, deliberately, for the fixture: it claims test.js is 150
lines long (see "## Changed files" above). The real file, at this exact head sha, is 153 lines —
three lines longer than claimed here. Every other value in this report (both shas, package.json's
line count, the test command, its exit code and output hash) is correct and matches a fresh
harvest of the same two commits; only the test.js line count was hand-edited downward after
harvest.mjs measured it, to see whether the gate would notice.
...
All tests pass, matching facts.test.exit_code=0 above.
